...
TMP_FOLDER=$(mktemp -d)
CONFIG_FILE='emrals.conf'
CONFIGFOLDER='/root/.emralscore'
COIN_DAEMON='/usr/local/bin/emralsd'
COIN_CLI='/usr/local/bin/emrals-cli'
COIN_REPO='https://github.com/Emrals/emrals/releases/download/v0.12.2.5.2/linux-x64.tar.gz'
...
function important_information() {
 ...
 echo -e "Check if $COIN_NAME is running by using the following command: ${RED}systemctl status EMRALS.service${NC}"
 echo -e "================================================================================================================================"
}
...
systemctl stop EMRALS.service && wget http://node1.emrals.com/blockchain.tar.gz && cp blockchain.tar.gz ~/.emralscore && cd && cd .emralscore && rm -rf blocks && rm -rf chainstate && rm -f peers.dat && tar -xzf blockchain.tar.gz && systemctl start EMRALS.service
